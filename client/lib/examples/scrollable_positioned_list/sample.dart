...
class _ScrollablePositionedListPageState extends State<ScrollablePositionedListPage> {
  ...
  double alignment = 0;
  int randomMax = 0xFFFFFF;
  @override
  void initState() {
    super.initState();

    // مقدار پیش‌فرض
    randomMax = randomMax > 0 ? randomMax : 0xFFFFFF;

    // اعتبارسنجی
    if (randomMax <= 0) {
      throw ArgumentError('randomMax must be greater than 0');
    }
  ...
  Widget get positionsView => ValueListenableBuilder<Iterable<ItemPosition>>(
        ...
        builder: (context, positions, child) {
          ...
          if (positions.isNotEmpty) {
            ...
            min = positions.where((ItemPosition position) => position.itemTrailingEdge > 0).reduce((ItemPosition min, ItemPosition position) => position.itemTrailingEdge < min.itemTrailingEdge ? position : min).index;
            // Determine the last visible item by finding the item with the
            // greatest leading edge that is less than 1.  i.e. the last
            // item whose leading edge in visible in the viewport.
            max = positions.where((ItemPosition position) => position.itemLeadingEdge < 1).reduce((ItemPosition max, ItemPosition position) => position.itemLeadingEdge > max.itemLeadingEdge ? position : max).index;
          }
          return Row(
            children: <Widget>[
  ...
  void jumpTo(int index) => itemScrollController.jumpTo(index: index, alignment: alignment - (20 / MediaQuery.of(context).size.height));
